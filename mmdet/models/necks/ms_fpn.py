...
class MSFPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.focus_conv = nn.ModuleList()
 

        for i in range(self.start_level, self.backbone_end_level):
            #首先利用focus结构，进行下采样，再利用1x1的卷积来调整通道数
            ...
            #focus_conv = Focus(in_channels[i], out_channels,k=1,s=1),
            #这里的输入通道应该填什么还有待考虑
            ...
            #self.focus_conv.append(focus_conv)
    ...
    def forward(self, inputs):
        ...
        for i in range(0,used_backbone_levels - 1,  1):
            ...
            if 'scale_factor' in self.downsample_cfg:
                ...
                laterals[i+1] += F.interpolate(
                    laterals[i], size=prev_shape, **self.downsample_cfg)
        # build outputs
